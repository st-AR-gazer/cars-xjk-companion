    ...
    Func<string, bool, Action?, Task<string?>>? selectGame = null)
{
    ...
    public async Task RunAsync(string pairingCode, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            while (!lifetime.IsCancellationRequested)
            {
                ...
                {
                    _runtime = _runtime with { Stage = "error", Message = error is HttpRequestException or OperationCanceledException
    ...
    private async Task PairAsync(string code, CancellationToken cancellationToken)
    {
        ...
        var result = await api.ClaimPairingAsync(new PairingClaim {
            PairingCode = code, DeviceId = settings.DeviceId, DisplayName = Environment.MachineName, DeviceToken = settings.DeviceToken
        }, cancellationToken);
        ...
        // Pairing is saved by this point, so a first heartbeat that fails must
        // not undo it. The heartbeat loop retries within seconds; throwing here
        // would end the session and send the reader back to a spent code.
        try
        {
    ...
    internal async Task<bool> PrepareGameAsync(bool automatic, CancellationToken cancellationToken)
    {
        ...
        _runtime = _runtime with { Stage = "starting", Message = "Checking the usual Trackmania folders.",
            GameReady = false, Installed = false, Files = [] };
        if (automatic)
        {
            // Discovery is read-only. Present every candidate to the browser;
            // finding an EXE never authorizes a download, recovery, or installation.
            ...
            _runtime = _runtime with { Stage = "ready", GameChoices = _gameChoices,
                SelectedGameId = confirmed ? selected!.Id : "", GameReady = confirmed,
        ...
        if (root is null)
        {
            _runtime = previous with { Stage = "ready", GameReady = settings.GameConfirmed && CompanionGame.IsRoot(settings.TrackmaniaRoot),
        ...
        settings.GameConfirmed = true; // The user explicitly selected this folder in the picker.
    ...
    private async Task InspectAsync(CancellationToken cancellationToken)
    {
        ...
        _runtime = _runtime with { Stage = "ready", Message = "Connected. Ready for website commands.", GameReady = true,
            Installed = inspection.Installed, Files = inspection.Files };
            ...
            async (notice, token) =>
            {
                ...
                try
                {
                    if (notice.CompanionUpdate)
                    {
                        ...
                        return; // The new executable confirms installation after replacing this host.
                    }
                    using var api = new CompanionApi(settings);
