...
{
    internal const string PublishedReleaseId = ThumbnailRelease.ReleaseId;
    internal const long PublishedReleaseByteSize = ThumbnailRelease.ReleaseByteSize;
    internal const string PublishedReleaseSha256 = ThumbnailRelease.ReleaseSha256;
    ...
    public async Task InstallAsync(Func<int, string, Task> report, CancellationToken cancellationToken, string carId = "")
    {
        ValidateRoot();
        await RecoverInstallAsync(cancellationToken);
        var release = await FetchReleaseAsync(cancellationToken);
        var files = await FetchPackageFilesAsync(release, cancellationToken, carId);
    ...
    public async Task<CleanupResult> CleanupAsync(Func<int, string, Task> report, CancellationToken cancellationToken, string carId = "")
    {
        ValidateRoot();
        await RecoverInstallAsync(cancellationToken);
        ...
        HashSet<string>? selectedPackages = null;
        if (carId.Length > 0)
        {
            var release = await FetchReleaseAsync(cancellationToken);
            selectedPackages = release.Packages.Where(package => package.CarId == carId)
                .Select(package => package.PackageId).ToHashSet(StringComparer.Ordinal);
            if (selectedPackages.Count == 0) throw new InvalidDataException("The car is not in this release.");
        }
        for (var index = 0; index < ownership.Files.Count; index++)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var file = ownership.Files[index];
            if (selectedPackages is not null && !selectedPackages.Contains(file.PackageId))
            {
                retained.Add(file);
                continue;
            }
    ...
    public async Task<SkinApplicationResult> ApplySkinAsync(string carId, string skinId, Func<int, string, Task> report, CancellationToken cancellationToken,
        Func<Task>? beforeCommit = null, Func<PreparedSkinCommit, Task<bool>>? nativeCommit = null)
    {
        ValidateRoot();
        await RecoverInstallAsync(cancellationToken);
        ...
        var beforeOwnership = JsonSerializer.Serialize(ownership, JsonOptions);
        ...
        await ValidateThumbnailsAsync(ownership, carId, cancellationToken);
        ...
        var liveryIcon = await SkinArchiveComposer.ReadIconAsync(liveryPath, cancellationToken);
        ...
        ownership.UpdatedAt = DateTimeOffset.UtcNow.ToString("O");
        if (nativeCommit is not null && await nativeCommit(new PreparedSkinCommit(
            [new(carId, partial, currentHash, currentIsOverlay ? previousOverlay!.ReplacementByteSize : owned.ByteSize, replacementSha256, replacementInfo.Length)],
            beforeOwnership, JsonSerializer.Serialize(ownership, JsonOptions))))
        {
            DeleteIfExists(partial);
            await SyncThumbnailsAsync(carId, liveryIcon, skinId, cancellationToken);
            return new SkinApplicationResult(liverySha256, liveryInfo.Length);
        }
        if (beforeCommit is not null) await beforeCommit();
        if (!File.Exists(destination) || await Sha256FileAsync(destination, cancellationToken) != currentHash)
            throw new IOException("The vehicle archive changed while the skin was prepared; it was preserved.");
    ...
    public async Task RestoreSkinAsync(string carId, Func<int, string, Task> report, CancellationToken cancellationToken, Func<Task>? beforeCommit = null, Func<PreparedSkinCommit, Task<bool>>? nativeCommit = null)
    {
        await RestoreSkinsAsync(carId, report, cancellationToken, beforeCommit, nativeCommit);
    }
    ...
    public async Task<InstallationStatus> InspectAsync(CancellationToken cancellationToken, bool recover = true)
    {
        ValidateRoot();
        if (recover) await RecoverInstallAsync(cancellationToken);
        var ownership = LoadOwnership();
        if (ownership is null) return new InstallationStatus(false, []);
        var expected = await FetchPackageFilesAsync(await FetchReleaseAsync(cancellationToken), cancellationToken);
        var expectedByPath = expected.ToDictionary(file => file.File.LogicalPath, StringComparer.OrdinalIgnoreCase);
        var statuses = new List<ManagedFileStatus>();
        var blockingModifications = 0;
        var adoptableMatches = 0;
        var unownedMissingFiles = 0;
        var ownedPaths = ownership.Files.Select(file => file.LogicalPath).ToHashSet(StringComparer.OrdinalIgnoreCase);
        foreach (var owned in ownership.Files)
        {
            var destination = ResolveManagedPath(owned.LogicalPath);
            var state = "missing";
            if (File.Exists(destination))
            {
                var size = new FileInfo(destination).Length;
                var hash = await Sha256FileAsync(destination, cancellationToken);
                var overlay = ownership.Overlays.SingleOrDefault(item => item.LogicalPath == owned.LogicalPath);
                state = size == owned.ByteSize && hash == owned.Sha256 ? "verified" :
                    overlay is not null && size == overlay.ReplacementByteSize && hash == overlay.ReplacementSha256 ? "skin" : "modified";
                if (state == "modified")
                {
                    if (expectedByPath.TryGetValue(owned.LogicalPath, out var currentReleaseFile) &&
                        !BlocksReleaseAdoption(size, hash, currentReleaseFile.File.ByteSize, currentReleaseFile.File.Sha256))
                        adoptableMatches++;
                    else blockingModifications++;
                }
            }
            statuses.Add(new ManagedFileStatus(owned.LogicalPath, state, owned.PackageId, owned.ByteSize));
        }
        foreach (var releaseFile in expected.Where(file => !ownedPaths.Contains(file.File.LogicalPath)))
        {
            var destination = ResolveManagedPath(releaseFile.File.LogicalPath);
            if (!File.Exists(destination)) { unownedMissingFiles++; continue; }
            var size = new FileInfo(destination).Length;
            var hash = await Sha256FileAsync(destination, cancellationToken);
            if (BlocksReleaseAdoption(size, hash, releaseFile.File.ByteSize, releaseFile.File.Sha256))
                blockingModifications++;
            else adoptableMatches++;
        }
        var installed = expected.All(file => ownership.Files.Any(owned => owned.LogicalPath == file.File.LogicalPath &&
            owned.Sha256 == file.File.Sha256 && owned.ByteSize == file.File.ByteSize)) &&
            statuses.All(file => file.State is "verified" or "skin");
        return new InstallationStatus(installed, statuses, ownership.ReleaseId, blockingModifications, adoptableMatches, unownedMissingFiles);
    }

    internal static bool BlocksReleaseAdoption(long currentSize, string currentHash, long targetSize, string targetHash) =>
        currentSize != targetSize || !StringComparer.Ordinal.Equals(currentHash, targetHash);

    private async Task<List<ReleaseFile>> FetchPackageFilesAsync(ReleaseManifest release, CancellationToken cancellationToken, string carId = "")
    {
        var files = new List<ReleaseFile>();
        var paths = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var packages = new Dictionary<string, PackageManifest>(StringComparer.Ordinal);
        foreach (var reference in release.Packages)
        {
            ...
            if (!packages.TryAdd(package.PackageId, package)) throw new InvalidDataException("Duplicate package ID.");
        ...
        if (carId.Length > 0)
        {
            var selected = packages.Values.Where(package => package.CarId == carId).Select(package => package.PackageId)
                .ToHashSet(StringComparer.Ordinal);
            if (selected.Count == 0) throw new InvalidDataException("The car is not in this release.");
            var queue = new Queue<string>(selected);
            while (queue.TryDequeue(out var id))
            {
                foreach (var dependency in packages[id].Dependencies)
                {
                    if (!packages.TryGetValue(dependency.PackageId, out var target) || target.PackageVersion != dependency.PackageVersion)
                        throw new InvalidDataException("A car dependency is unavailable.");
                    if (selected.Add(target.PackageId)) queue.Enqueue(target.PackageId);
                }
            }
            files.RemoveAll(file => !selected.Contains(file.PackageId));
        }
    ...
    private async Task InstallFileAsync(ReleaseFile releaseFile, OwnershipManifest ownership, CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(destination))
        {
            ...
            if (matchesOverlay && existing is not null && FactorySkinUpgrade.CanPreserve(existing, file, overlay!))
            {
                // Keep the chosen livery exactly as it is; factory restoration
                // now points at the newly pinned solid-colour base archive.
                ownership.Overlays[ownership.Overlays.IndexOf(overlay!)] = FactorySkinUpgrade.Rebase(overlay!, file);
                UpsertOwnership(ownership, releaseFile);
                SaveOwnership(ownership);
                return;
            }
    ...
    {
        await MoreCars.Native.NativeSkinStage.RecoverAsync(settings.TrackmaniaRoot, CompanionGame.IsRunning(settings.TrackmaniaRoot, true), cancellationToken);
        await RecoverSkinResetAsync(cancellationToken);
    ...
    private OwnershipManifest? LoadOwnership()
    {
        ...
        if (manifest.Schema != "morecars.ownership.v1" || manifest.Files.Count > 1024 || manifest.Overlays.Count > 64)
    ...
    {
        for (var current = Path.GetFullPath(path); !string.IsNullOrWhiteSpace(current); current = Path.GetDirectoryName(current))
        {
            try
            {
                if ((File.GetAttributes(current) & FileAttributes.ReparsePoint) != 0)
                    throw new IOException("A managed path contains a symbolic link or junction; it was preserved.");
            }
    ...
    private void ValidateRoot()
    {
        ...
        RequireNoLinks(Path.Combine(settings.TrackmaniaRoot, ".morecars", "ownership-v1.json"));
    }
