...
try {
  ...
  $buildInfo = @{ schema = "morecars.companion-build.v1"; version = [string]$declaredVersion; runtime = $Runtime; sha256 = $hash }
  [IO.File]::WriteAllText((Join-Path $resolvedOutput "companion-build.json"), (($buildInfo | ConvertTo-Json) + "`n"))
