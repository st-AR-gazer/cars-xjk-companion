...
internal static class SkinArchiveComposer
{
    ...
    // The livery's own preview is optional. A missing or unsupported icon uses
    // the separately packaged factory render instead of the base ZIP's old icon.
    public static async Task<byte[]?> ReadIconAsync(string liveryArchivePath, CancellationToken cancellationToken)
    {
        await using var stream = new FileStream(liveryArchivePath, FileMode.Open, FileAccess.Read, FileShare.Read);
        using var archive = new ZipArchive(stream, ZipArchiveMode.Read, false);
        if (archive.Entries.Count is < 1 or > 256) return null;
        var matches = archive.Entries.Where(entry =>
        {
            ValidateArchivePath(entry.FullName);
            return Path.GetFileName(entry.FullName).Equals("Icon.dds", StringComparison.OrdinalIgnoreCase);
        }).ToArray();
        if (matches.Length != 1 || matches[0].Length is < 128 or > 16 * 1024 * 1024 ||
            matches[0].Length > matches[0].CompressedLength * 200 + 1024 * 1024) return null;
        try
        {
            ...
            ValidateDds(bytes);
            if (BitConverter.ToUInt32(bytes, 16) > 2048 || BitConverter.ToUInt32(bytes, 12) > 2048) return null;
            return bytes;
        }
