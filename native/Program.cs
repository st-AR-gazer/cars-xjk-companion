    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            Console.WriteLine(JsonSerializer.Serialize(new {
                schema = "morecars.native-probe.v1", observedAt = DateTimeOffset.UtcNow,
                processId = memory.ProcessId, processStartedUtcTicks = memory.ProcessStartedUtcTicks,
                gameVersion = GameProfile.Version, executableSha256 = GameProfile.Sha256,
                snapshot, gameThreadDispatcherValidated = false,
        ...
        {
            Console.Error.WriteLine(JsonSerializer.Serialize(new { schema = "morecars.native-probe.v1", refreshEnabled = false, error = error.Message,
