...
void Need(bool success, DWORD code = 0) {
    if (!success)
        throw Failure{code ? code : GetLastError()};
}
struct Handle {
...
void NoLinks(const std::wstring &path) {
    ...
    while (!current.empty()) {
        ...
        if (attributes != INVALID_FILE_ATTRIBUTES)
            Need(!(attributes & FILE_ATTRIBUTE_REPARSE_POINT), ERROR_REPARSE_TAG_INVALID);
        else
...
Handle LockDirectory(const std::wstring &path) {
    ...
    Need(GetFileInformationByHandle(h.value, &info));
    Need((info.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
...
void Hash(HANDLE file, unsigned char result[32]) {
    ...
    Need(BCryptOpenAlgorithmProvider(&algorithm, BCRYPT_SHA256_ALGORITHM, nullptr, 0) >= 0,
         ERROR_CRC);
    try {
        Need(BCryptGetProperty(algorithm, BCRYPT_OBJECT_LENGTH, reinterpret_cast<PUCHAR>(&size),
                               sizeof(size), &actual, 0) >= 0 &&
                 size < 65536,
             ERROR_CRC);
        std::vector<unsigned char> object(size), buffer(65536);
        Need(BCryptCreateHash(algorithm, &hash, object.data(), size, nullptr, 0, 0) >= 0,
             ERROR_CRC);
        LARGE_INTEGER beginning{};
        Need(SetFilePointerEx(file, beginning, nullptr, FILE_BEGIN));
        while (true) {
            DWORD count = 0;
            Need(ReadFile(file, buffer.data(), static_cast<DWORD>(buffer.size()), &count, nullptr));
            if (!count)
                break;
...
Handle OpenVerified(const std::wstring &path, const unsigned char expected[32],
                    std::uint64_t size) {
    NoLinks(path);
    Handle h(CreateFileW(path.c_str(), GENERIC_READ | DELETE, FILE_SHARE_READ, nullptr,
                         OPEN_EXISTING, FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_SEQUENTIAL_SCAN,
                         nullptr));
    Need(h.value != INVALID_HANDLE_VALUE);
    BY_HANDLE_FILE_INFORMATION info{};
    Need(GetFileInformationByHandle(h.value, &info));
    Need(!(info.dwFileAttributes & (FILE_ATTRIBUTE_REPARSE_POINT | FILE_ATTRIBUTE_DIRECTORY)) &&
             info.nNumberOfLinks == 1,
         ERROR_REPARSE_TAG_INVALID);
    Need(((static_cast<std::uint64_t>(info.nFileSizeHigh) << 32) | info.nFileSizeLow) == size,
         ERROR_CRC);
...
bool Rollback(std::vector<Entry> &entries) {
    ...
    for (auto i = entries.rbegin(); i != entries.rend(); ++i) {
        try {
            if (i->newMoved && i->after.value == INVALID_HANDLE_VALUE)
                i->after = OpenVerified(i->target, i->file.afterHash, i->file.afterSize);
            if (i->oldMoved && i->before.value == INVALID_HANDLE_VALUE)
                i->before = OpenVerified(i->backup, i->file.beforeHash, i->file.beforeSize);
            if (i->newMoved) {
                const auto moved = Rename(i->after.value, i->source);
                complete = complete && moved;
                if (moved)
                    i->newMoved = false;
            }
            if (i->oldMoved) {
                const auto moved = Rename(i->before.value, i->target);
                complete = complete && moved;
                if (moved)
                    i->oldMoved = false;
            }
...
bool Validate(const Request &request) {
    if (request.schema != 1 || request.byteSize != sizeof(Request) || request.count < 2 ||
        request.count > 12 || request.files[request.count - 1].key != 11 || !request.ownerPid ||
        !request.ownerStartedAt || !request.expiresAt ||
        std::all_of(std::begin(request.id), std::end(request.id),
    ...
    for (std::uint32_t i = 0; i < request.count; ++i) {
        ...
        if (file.key > 11 || (i && file.key <= previous) || file.missingBefore > 1 ||
            file.beforeSize > maximum || !file.afterSize || file.afterSize > maximum ||
            (!file.missingBefore && !file.beforeSize) ||
            (file.missingBefore &&
             (file.key == 11 || file.beforeSize ||
              !std::all_of(std::begin(file.beforeHash), std::end(file.beforeHash),
                           [](auto b) { return b == 0; }))))
            return false;
        previous = file.key;
    }
...
std::wstring Id(const Request &request) {
    ...
    std::wstring id;
    id.reserve(32);
    for (auto value : request.id) {
        id.push_back(digits[value >> 4]);
        id.push_back(digits[value & 15]);
    }
    return id;
}
...
Result Commit(const std::wstring &root, const Request &request, Guard guard, Refresh refresh,
    ...
    try {
        ...
        const auto now = Now();
        Need(request.expiresAt > now && request.expiresAt - now <= 30ULL * 60 * 10000000,
             ERROR_TIMEOUT);
        const auto staging = root + L"\\.morecars\\native\\" + Id(request);
        for (const auto &path : {root, root + L"\\GameData", root + L"\\GameData\\Vehicles",
                                 root + L"\\GameData\\Vehicles\\Skins", root + L"\\.morecars",
                                 root + L"\\.morecars\\native", staging})
            directories.push_back(LockDirectory(path));
        ...
        for (std::uint32_t i = 0; i < request.count; ++i) {
            ...
            Need(GetFileAttributesW(entry.backup.c_str()) == INVALID_FILE_ATTRIBUTES &&
                     GetLastError() == ERROR_FILE_NOT_FOUND,
                 ERROR_ALREADY_EXISTS);
            entry.after = OpenVerified(entry.source, file.afterHash, file.afterSize);
            if (file.missingBefore) {
                NoLinks(entry.target);
                Need(GetFileAttributesW(entry.target.c_str()) == INVALID_FILE_ATTRIBUTES &&
                         GetLastError() == ERROR_FILE_NOT_FOUND,
                     ERROR_FILE_EXISTS);
            } else
                entry.before = OpenVerified(entry.target, file.beforeHash, file.beforeSize);
            entries.push_back(std::move(entry));
        }
        Need(guard(context) && request.expiresAt > Now(), ERROR_CANCELLED);
        for (auto &entry : entries) {
            if (entry.before.value != INVALID_HANDLE_VALUE) {
                Need(Rename(entry.before.value, entry.backup));
                entry.oldMoved = true;
            }
            Need(Rename(entry.after.value, entry.target));
            entry.newMoved = true;
        }
        for (auto &entry : entries) {
            entry.before.Reset();
