...
static bool RequestAlive() {
    ...
    return commitOwner && WaitForSingleObject(commitOwner, 0) == WAIT_TIMEOUT &&
           FileTimeValue(now) < commitRequest.expiresAt;
}
...
extern "C" __declspec(dllexport) DWORD WINAPI MoreCarsSubmitSkinCommit(void *input) {
    ...
    if (!CopyRequest(input, &request) || !SkinTransaction::Validate(request))
        return ERROR_INVALID_DATA;
    HANDLE owner =
        OpenProcess(SYNCHRONIZE | PROCESS_QUERY_LIMITED_INFORMATION, FALSE, request.ownerPid);
    FILETIME created{}, exited{}, kernel{}, user{};
    if (!owner)
        return GetLastError();
    if (!GetProcessTimes(owner, &created, &exited, &kernel, &user) ||
        FileTimeValue(created) != request.ownerStartedAt) {
        CloseHandle(owner);
        return ERROR_INVALID_OWNER;
    }
    AcquireSRWLockExclusive(&commitLock);
    DWORD result = 0;
    if (!MoreCarsProbeState.running || MoreCarsProbeState.mixedThreads || MoreCarsProbeState.fault)
        result = ERROR_NOT_READY;
    else if (MoreCarsCommitState.state == 1 || MoreCarsCommitState.state == 2)
        result = ERROR_BUSY;
    else {
        if (commitOwner)
            CloseHandle(commitOwner);
        commitOwner = owner;
        owner = nullptr;
        commitRequest = request;
        std::memcpy(MoreCarsCommitState.id, request.id, sizeof(request.id));
        MoreCarsCommitState.error = 0;
        MoreCarsCommitState.rollbackComplete = 1;
        MoreCarsCommitState.refreshed = 0;
        MoreCarsCommitState.threadId = 0;
...
extern "C" __declspec(dllexport) DWORD WINAPI MoreCarsCancelSkinCommit(void *input) {
    ...
    AcquireSRWLockExclusive(&commitLock);
    DWORD result = 0;
    if (std::memcmp(id, MoreCarsCommitState.id, sizeof(id)) != 0)
        result = ERROR_NOT_FOUND;
    else if (MoreCarsCommitState.state == 2)
        result = ERROR_BUSY;
...
static bool RuntimeMenu(void *self) {
    __try {
        return MoreCarsProbeState.running && !MoreCarsProbeState.mixedThreads &&
               !MoreCarsProbeState.fault && MenuState(self);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        return false;
    }
}
    ...
    __try {
        if (!RuntimeMenu(self) ||
            std::memcmp(reinterpret_cast<void *>(gameBase + Profile::UpdateTree),
                        Profile::UpdateTreePrefix, sizeof(Profile::UpdateTreePrefix)))
            return false;
        auto folder = GameDataFolder();
        if (!folder)
            return false;
        const auto update = reinterpret_cast<void(__fastcall *)(void *, unsigned int)>(
            gameBase + Profile::UpdateTree);
        update(reinterpret_cast<void *>(folder), 0);
        folder = FindFolder(GameDataFolder(), "Vehicles");
        if (!folder || !RuntimeMenu(self))
            return false;
...
static void ProcessSkinCommit(void *self) {
    if (!TryAcquireSRWLockExclusive(&commitLock))
        return;
    if (MoreCarsCommitState.state != 1) {
        ReleaseSRWLockExclusive(&commitLock);
        return;
    }
    if (!RequestAlive()) {
    ...
    try {
        ...
        if (length && length < 32768) {
            ...
            if (slash != std::wstring::npos)
                result = SkinTransaction::Commit(root.substr(0, slash), request, RuntimeCommitGuard,
                                                 RefreshCommittedSkins, self);
        }
    } catch (...) {
