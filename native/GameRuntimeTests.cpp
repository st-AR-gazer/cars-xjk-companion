...
int main() {
    try {
        ...
        gameBase = reinterpret_cast<std::uintptr_t>(image.data());
        gameApp = app.data();
        *reinterpret_cast<void **>(gameBase + Profile::AppSingleton) = gameApp;
        ...
        request.ownerStartedAt = FileTimeValue(created);
        request.expiresAt = FileTimeValue(now) + 600000000;
        request.files[0].key = 0;
        request.files[1].key = 11;
        ...
        Check(MoreCarsSubmitSkinCommit(&request) == ERROR_BUSY && MoreCarsCommitState.state == 1,
              "concurrent submission");
        *reinterpret_cast<void **>(app.data() + Profile::Playground) = app.data();
        ProcessSkinCommit(app.data());
        Check(MoreCarsCommitState.state == 1, "map defers transaction");
        *reinterpret_cast<void **>(app.data() + Profile::Playground) = nullptr;
        *reinterpret_cast<void **>(app.data() + Profile::Editor) = app.data();
        ProcessSkinCommit(app.data());
        Check(MoreCarsCommitState.state == 1, "editor defers transaction");
        unsigned char otherId[16]{};
        Check(MoreCarsCancelSkinCommit(otherId) == ERROR_NOT_FOUND &&
                  MoreCarsCommitState.state == 1,
              "foreign cancellation");
        Check(MoreCarsCancelSkinCommit(request.id) == 0 && MoreCarsCommitState.state == 5 &&
                  !commitOwner,
              "cancel closes owner handle");
        request.ownerStartedAt++;
        Check(MoreCarsSubmitSkinCommit(&request) == ERROR_INVALID_OWNER, "PID reuse rejected");
        request.ownerStartedAt--;
        Check(MoreCarsSubmitSkinCommit(&request) == 0, "resubmit");
        MoreCarsCommitState.state = 2;
        Check(!PrepareSkinRuntimeStop() && MoreCarsProbeState.running == 1 && commitOwner,
              "in-progress commit prevents stop");
        MoreCarsCommitState.state = 1;
        Check(PrepareSkinRuntimeStop() && !MoreCarsProbeState.running &&
                  MoreCarsCommitState.state == 5 && !commitOwner,
              "stop cancels pending commit");
        ...
        ProcessSkinCommit(app.data());
        Check(MoreCarsCommitState.state == 5 && !commitOwner, "expiry cancels before menu");
