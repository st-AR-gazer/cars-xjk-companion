...
if ($LASTEXITCODE -ne 0) { throw 'Native probe host build failed.' }
...
$taskSource = Join-Path $taskRoot 'FrameProbe.cpp'
$taskDll = Join-Path $taskOutput 'MoreCars.FrameProbe.dll'
$taskObj = Join-Path $taskArtifacts 'FrameProbe.obj'
$taskImportLib = Join-Path $taskArtifacts 'MoreCars.FrameProbe.lib'
$taskTests = Join-Path $taskRoot 'FrameProbeTests.cpp'
$taskTestExe = Join-Path $taskArtifacts 'MoreCars.FrameProbeTests.exe'
$taskTestObj = Join-Path $taskArtifacts 'FrameProbeTests.obj'
$taskBatch = @"
@echo off
call "$taskSetup" >nul
if errorlevel 1 exit /b 1
cl /nologo /std:c++20 /O2 /MT /EHsc /W4 /WX /guard:cf /LD /I"$taskArtifacts" "$taskSource" /Fo"$taskObj" /Fe"$taskDll" /link /INCREMENTAL:NO /guard:cf /IMPLIB:"$taskImportLib"
if errorlevel 1 exit /b 1
cl /nologo /std:c++20 /O2 /MT /EHsc /W4 /WX /guard:cf /I"$taskArtifacts" "$taskTests" /Fo"$taskTestObj" /Fe"$taskTestExe" /link /INCREMENTAL:NO /guard:cf
exit /b %errorlevel%
"@
[IO.File]::WriteAllText((Join-Path $taskArtifacts 'build-native.cmd'), $taskBatch, [Text.ASCIIEncoding]::new())
Push-Location $taskArtifacts
try { & $env:ComSpec /d /c build-native.cmd }
finally { Pop-Location }
if ($LASTEXITCODE -ne 0) { throw 'Native frame observer build failed.' }
& $taskTestExe
if ($LASTEXITCODE -ne 0) { throw 'Native frame observer checks failed.' }
