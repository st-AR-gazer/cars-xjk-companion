...
internal static class CompanionPlatform
{
    ...
    {
#if WINDOWS
        ShowForegroundDialog(owner =>
            MessageBox.Show(owner, message, ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Information));
#else
...
#if WINDOWS
        ShowForegroundDialog(owner =>
            MessageBox.Show(owner, message, ApplicationName, MessageBoxButtons.OK, MessageBoxIcon.Error));
#else
    ...
    {
#if WINDOWS
        return SelectWindowsTrackmaniaRoot(currentPath);
#else
...
#if WINDOWS
    internal static IReadOnlyList<string> CommonTrackmaniaCandidates(string programFiles, string programFilesX86)
    {
        var roots = new List<string>();
        foreach (var baseDirectory in new[] { programFiles, programFilesX86 })
        {
            if (string.IsNullOrWhiteSpace(baseDirectory)) continue;
            roots.Add(Path.Combine(baseDirectory, "Ubisoft", "Ubisoft Game Launcher", "games", "Trackmania"));
            roots.Add(Path.Combine(baseDirectory, "Steam", "steamapps", "common", "Trackmania"));
        }
        return roots.Distinct(PathComparer).ToArray();
    }

    internal static string? DetectCommonTrackmaniaRoot()
    {
        var candidates = CommonTrackmaniaCandidates(
                Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles),
                Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86))
            .Where(path => File.Exists(Path.Combine(path, "Trackmania.exe")) &&
                           Directory.Exists(Path.Combine(path, "GameData")))
            .ToArray();
        return candidates.Length == 1 ? Path.GetFullPath(candidates[0]) : null;
    }

    private static string? SelectWindowsTrackmaniaRoot(string currentPath)
    {
        ...
        return ShowForegroundDialog(owner => picker.ShowDialog(owner)) == DialogResult.OK ? picker.SelectedPath : null;
    }

    // A visible taskbar owner keeps manually opened pickers discoverable even
    // when Windows does not transfer focus from the browser.
    internal static Form CreateDialogOwner()
    {
        var owner = new Form
        {
            FormBorderStyle = FormBorderStyle.FixedDialog,
            ClientSize = new System.Drawing.Size(420, 100),
            ShowIcon = true,
        ...
        owner.Controls.Add(new Label
        {
            ...
            Text = "Finish or cancel the open dialog to return to the website."
        });
        return owner;
    }

    private static T ShowForegroundDialog<T>(Func<IWin32Window, T> showDialog)
    {
    ...
    {
        // Native update fixtures use a separate data home and leave the user's
        // registered protocol untouched while exercising the real installer.
        if (Environment.GetEnvironmentVariable("MORECARS_SKIP_PROTOCOL") == "1" &&
            !string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("MORECARS_DATA_HOME"))) return;
    ...
    public static void RemoveAfterCurrentProcessExits(IReadOnlyList<string> targets)
    {
        ...
        StartDelayedRemoval(targets);
    }

    internal static void RemoveUninstallHelperAfterExit(string directory)
    {
        CompanionUninstaller.RequireHelperDirectory(Path.Combine(directory, ExecutableFileName));
        StartDelayedRemoval([directory]);
    }

    private static void StartDelayedRemoval(IReadOnlyList<string> targets)
    {
