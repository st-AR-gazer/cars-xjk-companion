...
internal static class CompanionNativeSkins
{
        ...
        Func<int, string, Task> report, CancellationToken token)
    {
        ...
        {
            // No files have been staged/submitted. Unsupported builds and denied
            // process access retain the existing closed-game installation path.
            await report(700, "Live skin refresh is unavailable for this game session. Close Trackmania to finish applying the skin.");
            return null;
        }
        await using (client)
        {
            // Once submitted, never retry through the offline writer: an uncertain
            // game transaction must recover from its journal first.
