...
internal static class CompanionSelfTest
{
    ...
    private static async Task<bool> ThumbnailSidecarsAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            {
                InstallationId = "thumbnail-selftest", ReleaseId = ThumbnailRelease.ReleaseId,
                ReleaseManifestSha256 = ThumbnailRelease.ReleaseSha256,
                InstalledAt = DateTimeOffset.UtcNow.ToString("O"), UpdatedAt = DateTimeOffset.UtcNow.ToString("O")
            };
            foreach (var suffix in suffixes)
            {
                ...
                {
                    LogicalPath = logicalPath, Sha256 = Hash(factory), ByteSize = factory.Length,
                    PackageId = "bay-vehicle", PackageVersion = "1.2.0", Role = "vehicle-thumbnail"
                });
            }
    ...
    {
#if WINDOWS
        // A headless self-test has no WinForms message loop. Creating the dialog
        // owner must not capture later async continuations onto that context.
